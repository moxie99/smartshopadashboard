...
const Category = () => {
    ...
    return (
        <div className='px-2 lg:px-7 pt-5'>
            <div className='flex lg:hidden justify-between items-center mb-5 p-4 bg-secondary rounded-md'>
            ...
            <div className='flex flex-wrap w-full'>
                    ...
                    <div className='w-full p-4  bg-primary rounded-md'>
                        ...
                        <div className='relative overflow-x-auto'>
                            <table className='w-full text-sm text-left text-white'>
                                <thead className='text-sm text-white uppercase border-b border-secondary'>
                                    <tr>
                ...
                <div className={`w-[320px] lg:w-5/12 translate-x-100 lg:relative lg:right-0 fixed ${show ? 'right-0' : '-right-[340px]'} z-[9999] top-0 transition-all duration-500`}>
                    <div className='w-full pl-5'>
                        <div className='bg-primary h-screen lg:h-auto px-3 py-2 lg:rounded-md text-white'>
                            <div className='flex justify-between items-center mb-4'>
                                <h1 className='text-white font-semibold text-xl'>Add Category</h1>
                                <div onClick={() => setShow(false)} className='block lg:hidden cursor-pointer'><GrClose className='text-white' /></div>
                            </div>
                            <form onSubmit={add_category}>
                                <div className='flex flex-col w-full gap-1 mb-3'>
                                    <label htmlFor="name">Category name</label>
                                    <input value={state.name} onChange={(e) => setState({ ...state, name: e.target.value })} className='px-4 py-2 focus:primary outline-none bg-primary border border-secondary rounded-md text-white' type="text" id='name' name='category_name' placeholder='category name' required />
                                </div>
                                <div>

                                    <label className='flex justify-center items-center flex-col h-[238px] cursor-pointer border border-dashed hover:border-primary w-full border-white' htmlFor="image">
                                        {
                                            imageShow ? <img className='w-full h-full' src={imageShow} alt='catImg' /> : <>
                                                <span><BsImage /></span>

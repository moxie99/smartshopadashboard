...
const Payments = () => {
    ...
    const Row = ({ index, style }) => {
        return (
            <div style={style} className='flex text-sm'>
                ...
                <div className='w-[25%] p-2 whitespace-nowrap'>NGN{pendingWithdrows[index]?.amount}</div>
                <div className='w-[25%] p-2 whitespace-nowrap'>
                    <span className='py-[1px] px-[5px] bg-secondary text-blue-500 rounded-md text-xs'>{pendingWithdrows[index]?.status}</span>
                </div>
    ...
    const Rows = ({ index, style }) => {
        return (
            <div style={style} className='flex text-sm'>
                <div className='w-[25%] p-2 whitespace-nowrap'>{index + 1}</div>
                <div className='w-[25%] p-2 whitespace-nowrap'>NGN{successWithdrows[index]?.amount}</div>
                <div className='w-[25%] p-2 whitespace-nowrap'>
                    <span className='py-[1px] px-[5px] bg-secondary text-blue-500 rounded-md text-xs'>{successWithdrows[index]?.status}</span>
                </div>
    ...
    return (
        <div className='px-2 md:px-7 py-5'>
            <div className='w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-5'>
                <div className='flex justify-between items-center p-5 bg-primary rounded-md gap-3'>
                    <div className='flex flex-col justify-start items-start text-white'>
                        <h2 className='text-lg font-bold'>NGN{totalAmount}</h2>
                ...
                <div className='flex justify-between items-center p-5 bg-primary rounded-md gap-3'>
                    <div className='flex flex-col justify-start items-start text-white'>
                        <h2 className='text-lg font-bold'>NGN{availableAmount}</h2>
                ...
                <div className='flex justify-between items-center p-5 bg-primary rounded-md gap-3'>
                    <div className='flex flex-col justify-start items-start text-white'>
                        <h2 className='text-lg font-bold'>NGN{withdrowAmount}</h2>
                ...
                <div className='flex justify-between items-center p-5 bg-primary rounded-md gap-3'>
                    <div className='flex flex-col justify-start items-start text-white'>
                        <h2 className='text-lg font-bold'>NGN{pendingAmount}</h2>
            ...
            <div className='w-full grid grid-cols-1 md:grid-cols-2 gap-3 pb-4'>
                <div className='bg-primary  text-white rounded-md p-5'>
                    <h2 className='text-lg'>Send withdrawal Request</h2>
                    <div className='py-5'>
                        <form onSubmit={sendRequest}>
                            <div className='flex gap-3 flex-wrap'>
                                <input onChange={(e) => setAmount(e.target.value)} required value={amount} min='0' type="number" className='px-3 md:w-[79%] py-2 focus:primary outline-none bg-primary border border-secondary rounded-md text-white' name='amount' />
                    ...
                    <div>
                        ...
                        <div className='w-full overflow-x-auto'>
                            <div className='flex bg-secondary uppercase text-xs min-w-[340px]'>
